...
def draw_grid(grid):
    for row in range(SIZE):
        for column in range(SIZE):
            if "c" not in str(grid[row][column]) and "f" not in str(grid[row][column]) and "w" not in str(grid[row][column]): #DETTA STATEMENT ÄR ÄNDRAT!
                ...
                screen.blit(text, text_rect)
            elif "w" == grid[row][column]: #HELA DENNA FANNS INTE INNAN
                font = pygame.font.Font(None, 36)
                text = font.render("f", True, RED)  # Access grid[row][column] directly
                text_rect = text.get_rect(center=((MARGIN + CELL_SIZE) * column + MARGIN + CELL_SIZE / 2,
                                                  (MARGIN + CELL_SIZE) * row + HEADER_HEIGHT + MARGIN + CELL_SIZE / 2))
                screen.blit(text, text_rect)
    pygame.display.flip()
...
def lose(grid):
    for row in range(SIZE):
        for column in range(SIZE):
            ...
            elif "f" in str(grid[row][column]) and "x" not in str(grid[row][column]): #DETTA OCH NEDANFÖR FANNS INTE INNAN
                grid[row][column] = "w"  # Mark all wrongly flagged spots
